...
from connectors.farmax.farmax_status_tracker import (
    FarmaxStatusTracker,
    FarmaxTrackerConfig,
)
...
def test_start_initializes_loop(tracker):
    ...
    with patch.object(tracker, "_execute_poll_cycle") as mock_poll:
        ...
        tracker._poll_timer.start.assert_called_once_with(
            tracker._config.poll_interval_ms
        )
...
def test_poll_cycle_aborts_if_busy(tracker, mock_persistence):
    ...
    mock_persistence.get_active_monitored_ids.assert_not_called()
    tracker._logger.debug.assert_called_with(
...
@patch(
    "connectors.farmax.farmax_status_tracker.FarmaxWorker"
)  # Patch the worker class definition
def test_poll_cycle_batches_and_spawns_workers(
    mock_worker_cls, tracker, mock_persistence, mock_thread_pool
):
    """
    Happy Path:
...
def test_poll_cycle_resilience_to_exceptions(tracker, mock_persistence):
    ...
    tracker._is_running = True
    mock_persistence.get_active_monitored_ids.side_effect = Exception(
        "DB Connection Lost"
    )
