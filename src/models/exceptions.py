...
class TokenPollingError(AuthorizationError):
    ...
    def __init__(self, error_code: str, error_description: str = ""):
        ...
        super().__init__(
            f"Erro no polling do OAuth: {error_code} - {error_description}"
        )


class TokenStorageError(AuthorizationError):
    """Raised when the token cannot be accessed 
    (read/write/decode) on the file system."""

    def __init__(
        self,
        original_exception: Exception,
        message: str = "Falha em armazenar token no disco",
    ):
        self.original_exception = original_exception
