...
class Velide:
    ...
    async def delete_delivery(self, delivery_id: str) -> bool:
        ...
        payload = GraphQLPayload(
            query=self.DELETE_DELIVERY_MUTATION, variables=variables
        )
    ...
    @async_retry(operation_desc="buscar entregadores", max_retries=3)
    ...
    def _calculate_offset(self, created_at_time: datetime) -> int:
        ...
        # attach the expected timezone (e.g., UTC) 
        # rather than relying on system local time.
    ...
    def _flatten_snapshot(
        ...
        # Pydantic ensures 'data.deliveries' is a list 
        # (never None) due to default_factory
    ...
    async def _execute_request(self, payload: GraphQLPayload) -> httpx.Response:
        ...
        if self._client is None:
            raise RuntimeError(
                "Client is not initialized. Ensure you are " \
                "using 'async with Velide(...)'."
            )
    ...
    def _parse_response(
        self, response: httpx.Response, data_key: Optional[str] = None
    ) -> Any:
        """
        Generically parse and validate a GraphQL response, 
        extracting data from a specific key.

        Args:
            response: HTTP response from the API
            data_key: The key within the 'data' object 
            to extract (e.g., 'addDeliveryFromIntegration')
        ...
        try:
            ...
            # This catches if graphql_response.data doesn't 
            # have the attribute `data_key`
            raise GraphQLResponseError(
                f"Expected data key '{data_key}' not found "
                f"in response data: {response_json}"
            )
