...
class SQLiteService(QObject):
    ...
    def _create_and_run_worker(self, factory_method, *args, result_signal: Any):
        """
    ...
    def request_add_mapping(self, velide_id: str, local_id: str):
        ...
        self.logger.debug(
            f"Solicitando para adicionar mapeamento: {velide_id} -> {local_id}"
        )
    ...
    def request_add_many_mappings(self, mappings: List[Tuple[str, str]]):
        ...
        self.logger.debug(
            f"Solicitando {len(mappings)} mapeamentos para serem adicionados."
        )
        self._create_and_run_worker(
    ...
    def request_get_local_id(self, velide_id: str):
        ...
        self._create_and_run_worker(
            SQLiteWorker.for_get_local_id, velide_id, result_signal=self.local_id_found
        )
    ...
    def request_get_velide_id(self, local_id: str):
        ...
        self._create_and_run_worker(
            SQLiteWorker.for_get_velide_id, local_id, result_signal=self.velide_id_found
        )
    ...
    def request_get_all_mappings(self):
        ...
        self._create_and_run_worker(
            SQLiteWorker.for_get_all_mappings, result_signal=self.all_mappings_found
        )
    ...
    def request_add_delivery_mapping(self, external_id: str, internal_id: str, status):
        ...
        self.logger.debug(
            f"Solicitando adicionar entrega: {external_id} -> {internal_id} ({status})"
        )
        self._create_and_run_worker(
    ...
    @pyqtSlot(list)
    def request_add_many_delivery_mappings(
        self, mappings: List[Tuple[str, str, object]]
    ):
        """
        ...
        """
        self.logger.debug(
            f"Solicitando adicionar {len(mappings)} mapeamentos de entrega."
        )
        self._create_and_run_worker(
    ...
    @pyqtSlot(
        str, object, str
    )  # str, Enum, str (Optional is technically just `object` or `str` in signals)
    def request_update_delivery_status(
        self, external_id: str, new_status, deliveryman_id: Optional[str] = None
    ):
        """
        Asynchronously updates the status of a delivery.
        """
        self.logger.debug(
            f"Solicitando atualização: {external_id} -> "
            f"{new_status} (Entregador: {deliveryman_id})"
        )
        self._create_and_run_worker(
    ...
    def request_get_delivery_by_external(self, external_id: str):
        ...
        self.logger.debug(
            f"Solicitando busca de entrega por external ID: {external_id}"
        )
        self._create_and_run_worker(
    ...
    def request_get_delivery_by_internal(self, internal_id: str):
        ...
        self.logger.debug(
            f"Solicitando busca de entrega por internal ID: {internal_id}"
        )
        self._create_and_run_worker(
    ...
    def request_get_all_deliveries(self):
        ...
        self._create_and_run_worker(
            SQLiteWorker.for_get_all_deliveries, result_signal=self.all_deliveries_found
        )
