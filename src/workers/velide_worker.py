    ...
    def run(self):
        ...
        # --- Unified Exception Handling ---
            ...
            self.signals.error.emit(
                f"Erro de Rede: Não foi possível conectar à {e.request.url}"
            )

        except Exception:
            self.logger.exception(
                f"Ocorreu uma falha inesperada durante a operação: {self._operation}."
            )
    ...
    async def _run_async(self):
        ...
        async with self._velide as client:
            if self._operation == "add_delivery":
                ...
                if not order:
                    raise ValueError(
                        "Uma 'order' é necessária para a operação 'add_delivery'."
                    )

                response = await client.add_delivery(order)
                self.logger.info(
                    f"Nova entrega adicionada: {response.location.properties.name}"
                )
                ...
                self.logger.info(
                    f"Busca de entregadores concluída. {len(result)} encontrados."
                )
                ...
                raise NotImplementedError(
                    f"Operação desconhecida do VelideWorker: {self._operation}"
                )
