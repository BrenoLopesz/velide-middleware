...
def _decode_string_base64(encoded_string: str) -> str:
    ...
    try:
        ...
        raise TokenStorageError(
            message="Falha ao decodificar o token.", original_exception=e
        ) from e
...
def store_token_at_file(token: AccessTokenDict):
    ...
    try:
        ...
        raise TokenStorageError(
            message="Token não é um JSON válido.", original_exception=e
        ) from e
...
def read_token_from_file() -> Optional[dict]:
    ...
    try:
        ...
        raise TokenStorageError(
            message="Falha ao decodificar JSON do arquivo de token.",
