...
class FarmaxTrackerConfig:
    ...
    poll_interval_ms: int = 60 * 1000  # 60 Seconds (Checks are less urgent than new orders)
...
class FarmaxStatusTracker(QObject):
    ...
    def _execute_poll_cycle(self) -> None:
        ...
        if self._is_processing:
            self._logger.debug("Ciclo de rastreamento anterior ainda em andamento. Pulando...")
            return
        ...
        try:
            ...
            self._logger.info(f"Monitorando status de {len(active_ids)} pedidos ativos...")
    ...
    def _spawn_worker_for_batch(self, batch_ids: List[float]) -> None:
        ...
        # For simplicity in this non-blocking UI context, we release semaphore on finish.
        # Ideally, we should track active workers count, but a simple release on success is usually fine 
        ...
        try:
            for sale in sales_updates:
                ...
                if self._is_cancelled(raw_status):
                    ...
                    self._logger.info(f"Cancelamento detectado no Farmax para o pedido {internal_id_str} (Status: {raw_status}).")
                    ...
                    # Emit both IDs. 
                    # If external_id is None, DeliveriesService will handle it (remove from queue only).
