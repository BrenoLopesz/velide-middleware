...
class FarmaxDeliveryUpdater(QObject):
    ...
    # Signals to notify the Strategy of completion/failure 
    ...
    def mark_as_in_route(self, order: Order, deliveryman_external_id: str) -> None:
        ...
        driver = self._deliverymen_retriever.get_deliveryman_by_external_id(
            deliveryman_external_id
        )
        if not driver:
            self._logger.error(
                f"Não foi possível definir uma entrega ({order.internal_id}) "
                "como rota iniciada! Entregador não foi encontrado."
            )
            return
        ...
        worker.signals.success.connect(
            lambda: self._on_success(order.internal_id, "IN_ROUTE")
        )
        worker.signals.error.connect(
            lambda err: self._on_error(order.internal_id, "IN_ROUTE", err)
        )
    ...
    def mark_as_done(self, order: Order) -> None:
        ...
        worker = FarmaxWorker.for_update_delivery_as_done(
            self._repository, sale_id=sale_id, ended_at=current_time
        )

        worker.signals.success.connect(
            lambda: self._on_success(order.internal_id, "DONE")
        )
        worker.signals.error.connect(
            lambda err: self._on_error(order.internal_id, "DONE", err)
        )
    ...
    def _on_success(self, internal_id: str, operation: str):
        self._logger.debug(
            f"Sucesso ao atualizar Farmax: Pedido {internal_id} -> {operation}"
        )
        self.update_success.emit(internal_id, operation)

    def _on_error(self, internal_id: str, operation: str, error: str):
        self._logger.error(
            f"Erro ao atualizar Farmax ({operation}) Pedido {internal_id}: {error}"
        )
