...
class FarmaxWorker(QRunnable):
    ...
    def __init__(
        self, repository: FarmaxRepository, method_name: str, *args: Any, **kwargs: Any
    ):
        """
    ...
    def run(self):
        ...
        try:
            ...
            error_msg = f"Falha ao executar '{self.method_name}' no Farmax."
    ...
    @classmethod
    def for_fetch_deliverymen(cls, repository: FarmaxRepository) -> "FarmaxWorker":
        """Creates a worker to fetch all active deliverymen."""
        return cls(repository, "fetch_deliverymen")

    @classmethod
    def for_fetch_recent_changes(
        cls, repository: FarmaxRepository, last_check_time: datetime
    ) -> "FarmaxWorker":
        """Creates a worker to fetch delivery log changes."""
        # We pass arguments as kwargs for clarity and safety
        return cls(repository, "fetch_recent_changes", last_check_time=last_check_time)

    @classmethod
    def for_fetch_recent_changes_by_id(
        cls, repository: FarmaxRepository, last_id: int
    ) -> "FarmaxWorker":
        """Creates a worker to fetch delivery log changes, 
        based on latest fetched ID."""
        # We pass arguments as kwargs for clarity and safety
        return cls(repository, "fetch_recent_changes_by_id", last_id=last_id)

    @classmethod
    def for_fetch_deliveries_by_id(
        cls, repository: FarmaxRepository, cd_vendas: Tuple[float, ...]
    ) -> "FarmaxWorker":
        """Creates a worker to fetch full delivery details by ID."""
        return cls(repository, "fetch_deliveries_by_id", cd_vendas=cd_vendas)
    ...
    def for_fetch_sales_statuses_by_id(
        ...
        return cls(repository, "fetch_sales_statuses_by_id", cd_vendas=cd_vendas)
    ...
    def for_update_delivery_as_done(
        cls, repository: FarmaxRepository, sale_id: float, ended_at: time
    ) -> "FarmaxWorker":
        """Creates a worker to update a delivery to 'Done'."""
        return cls(
            repository, "update_delivery_as_done", sale_id=sale_id, ended_at=ended_at
        )
