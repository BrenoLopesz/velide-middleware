...
class SignatureVerifyService(QObject):
    ...
    def start_verification(
        self, directory: str, manifest_path: str, public_key_path: str
    ):
        """
        ...
        if not all(
            ...
            err_msg = (
                "Um ou mais caminhos são inválidos. "
                "Por favor, verifique os caminhos do diretório, "
                "da assinatura e do arquivo de chave."
            )
        ...
        self.logger.info(
            f"Worker de verificação para '{directory}' enviado para o pool de threads."
        )
