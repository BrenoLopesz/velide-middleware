...
def verify_directory(directory: str, manifest_path: str, public_key_path: str):
    ...
    logging.info(
        f"Attempting to verify manifest '{manifest_path}' "
        f"with signature '{signature_path}'..."
    )
    ...
    try:
        ...
        logging.critical(
            f"Required file not found: {e.filename}. Cannot trust the manifest."
        )
        return False, []
    except InvalidSignature:
        logging.critical(
            "❌ MANIFEST TAMPERED: Signature mismatch. " \
            "The manifest file cannot be trusted."
        )
        return False, []
    except Exception as e:
        logging.critical(
            f"An unexpected error occurred during manifest verification: {e}"
        )
        return False, []
    ...
    try:
        ...
        manifest_hashes = {
            item["hash"]: item["filename"] for item in manifest_data["files"]
        }
        ...
        try:
            ...
            if current_hash in manifest_hashes:
                ...
                inconsistencies.append(
                    f"'{relative_path}': UNTRUSTED (File is not listed in the manifest)"
                )
        except Exception as e:
            inconsistencies.append(
                f"'{relative_path}': FAILED (Could not process file: {e})"
            )
    ...
    if not inconsistencies:
        print(
            "\n✅ Success: All local files have been " \
            "verified successfully against the manifest."
        )
        if missing_files:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="A CLI tool to verify files in a " \
        "directory against a signed manifest."
    )
